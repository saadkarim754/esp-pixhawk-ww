/**
 * @file web-arming.c
 * @brief ESP32 MAVLink Companion Computer - Web Interface with Arming Support
 * 
 * Full-featured web control interface for Pixhawk:
 ...
 * - RC channel override for throttle-low before arming
 * - Status logs display with verbose diagnostics
 * 
 * Key fixes vs original webassistance.c:
 * - COMPANION_SYSTEM_ID = 200 (was 1, same as Pixhawk - caused Pixhawk to ignore all commands!)
 * - PARAM_SET to disable ARMING_CHECK, FS_THR_ENABLE, FS_GCS_ENABLE
 * - RC_CHANNELS_OVERRIDE to set throttle low before arming
 * - Force ARM with magic number 21196
 * - SET_MODE message (MSG_ID=11) for more reliable mode changes
 * 
 * Configuration:
...
// CRITICAL: Companion must have DIFFERENT system ID than Pixhawk!
// Using sysid=1 (same as Pixhawk) causes Pixhawk to drop all our commands.
...
/**
 * @brief Send ARM or DISARM command
 * @param arm true=ARM, false=DISARM
 * @param force true=force arm with magic number (bypasses all checks)
 */
static void send_arm_command(bool arm, bool force) {
    ...
    uint16_t len = mavlink_msg_command_long_pack(
        ...
        force_param,    // param2: 21196=force arm
    ...
    ESP_LOGI(TAG, "Sent %s command (from sysid=%d to sysid=%d)", cmd, COMPANION_SYSTEM_ID, PIXHAWK_SYSTEM_ID);
}

/**
 * @brief Send mode change using SET_MODE message (MSG_ID=11)
 * This is the same method pymavlink uses - more reliable than COMMAND_LONG
 */
static void send_mode_command(uint32_t mode) {
    uint8_t buf[32];
    
    // base_mode must have MAV_MODE_FLAG_CUSTOM_MODE_ENABLED set
    ...
    ESP_LOGI(TAG, "Sent SET_MODE: %s (%lu) base_mode=0x%02X", get_mode_name(mode), mode, base_mode);
}

/**
 * @brief Send PARAM_SET message to change a Pixhawk parameter
 */
...
/**
 * @brief Send RC_CHANNELS_OVERRIDE to set throttle low (required before arming)
 * Channel 3 = throttle, 1000us = minimum
...
static void send_rc_override_throttle_low(void) {
    ...
    uint16_t len = mavlink_msg_rc_channels_override_pack(
        ...
        0,      // chan4: don't override
        0, 0, 0, 0  // chan5-8: don't override
    );
    
    send_mavlink_message(buf, len);
    ESP_LOGI(TAG, "Sent RC_CHANNELS_OVERRIDE: CH3=1000 (throttle low)");
}

/**
 * @brief Disable pre-arm checks by setting Pixhawk parameters
 * Mirrors the RPi Python setup that works:
 *   ARMING_CHECK=0, FS_THR_ENABLE=0, FS_GCS_ENABLE=0
 */
...
static void handle_heartbeat(const mavlink_message_t *msg) {
    // Ignore heartbeats from GCS (Mission Planner = sysid 255)
    if (msg->sysid == 255) return;
    // Ignore our own heartbeats if somehow echoed
    ...
    // Log heartbeat periodically (every 10th = ~10 seconds)
    if (heartbeat_count % 10 == 1) {
        ESP_LOGI(TAG, "PIXHAWK: Mode=%s Status=%s %s (base_mode=0x%02X)",
...
static void handle_command_ack(const mavlink_message_t *msg) {
    ...
    if (ack.command == MAV_CMD_COMPONENT_ARM_DISARM) {
        if (ack.result == MAV_RESULT_ACCEPTED) {
            ...
            ESP_LOGI(TAG, "*** ARM/DISARM ACCEPTED ***");
        } else {
            add_log("ARM/DISARM FAILED: %s", result_str);
            ESP_LOGW(TAG, "*** ARM/DISARM FAILED: %s ***", result_str);
        }
    ...
    ESP_LOGI(TAG, "COMMAND_ACK: cmd=%d result=%s (from sysid=%d)", 
             ack.command, result_str, msg->sysid);
}
...
static void handle_param_value(const mavlink_message_t *msg) {
    ...
    // Null-terminate param_id safely
    char safe_id[17];
    memcpy(safe_id, param.param_id, 16);
    safe_id[16] = '\0';
    
    add_log("PARAM: %s = %.2f", safe_id, param.param_value);
    ESP_LOGI(TAG, "PARAM_VALUE: %s = %.4f (type=%d, index=%d/%d)", 
             safe_id, param.param_value, param.param_type, 
...
static void handle_statustext(const mavlink_message_t *msg) {
    ...
    // Stop at first non-printable character (garbage data after real text)
    char safe_text[51];
    int len = 0;
...
static void mavlink_rx_task(void *pvParameters) {
    ...
    while (1) {
        ...
        if (is_connected && (now - last_heartbeat_time > 3000)) {
            ...
            ESP_LOGW(TAG, "Pixhawk connection lost (no heartbeat for 3s)");
        }
    }
}

static void mavlink_heartbeat_task(void *pvParameters) {
    ESP_LOGI(TAG, "MAVLink heartbeat task started (sysid=%d compid=%d)", 
...
".btn{flex:1;padding:12px;border:none;border-radius:8px;font-size:14px;font-weight:bold;cursor:pointer;min-width:80px}\n"
".btn-arm{background:#f44;color:#fff}\n"
".btn-force{background:#f80;color:#fff}\n"
".btn-disarm{background:#4a4;color:#fff}\n"
".btn-setup{background:#44f;color:#fff}\n"
".btn:disabled{opacity:0.5;cursor:not-allowed}\n"
"select{width:100%;padding:12px;font-size:16px;border-radius:8px;border:none;background:#0e4d92;color:#fff;margin-top:8px}\n"
".logs{background:#0a0a15;border-radius:8px;padding:10px;height:180px;overflow-y:auto;font-family:monospace;font-size:12px}\n"
...
"<h2>CONTROLS</h2>\n"
"<div class='controls'>\n"
"<button class='btn btn-setup' onclick='setup()'>SETUP</button>\n"
"<button class='btn btn-arm' onclick='arm()'>ARM</button>\n"
"<button class='btn btn-force' onclick='forceArm()'>FORCE ARM</button>\n"
...
"function setup(){fetch('/api/setup',{method:'POST'}).then(()=>update());}\n"
"function arm(){fetch('/api/arm',{method:'POST'}).then(()=>update());}\n"
"function forceArm(){fetch('/api/forcearm',{method:'POST'}).then(()=>update());}\n"
"function disarm(){fetch('/api/disarm',{method:'POST'}).then(()=>update());}\n"
"function setMode(){var m=document.getElementById('modeSelect').value;if(m)fetch('/api/mode?m='+m,{method:'POST'}).then(()=>update());document.getElementById('modeSelect').value='';}\n"
...
/**
 * @brief SETUP handler - disables pre-arm checks via PARAM_SET
 */
...
/**
 * @brief ARM handler - sends RC override (throttle low) then arm command
 */
static esp_err_t arm_handler(httpd_req_t *req) {
    ESP_LOGI(TAG, "=== ARM requested from web ===");
    
    // Step 1: Set throttle low via RC override
...
/**
 * @brief FORCE ARM handler - bypasses all pre-arm checks
 */
static esp_err_t force_arm_handler(httpd_req_t *req) {
    ESP_LOGI(TAG, "=== FORCE ARM requested from web ===");
    
    // Step 1: Set throttle low via RC override
    send_rc_override_throttle_low();
    add_log("RC Override: throttle low");
    
    // Step 2: Force arm with magic number 21196
...
static esp_err_t disarm_handler(httpd_req_t *req) {
    ESP_LOGI(TAG, "=== DISARM requested from web ===");
...
static esp_err_t mode_handler(httpd_req_t *req) {
    char buf[32];
    ESP_LOGI(TAG, "=== MODE CHANGE requested from web ===");
...
static httpd_handle_t start_webserver(void) {
    ...
    if (httpd_start(&server, &config) == ESP_OK) {
        ...
        ESP_LOGI(TAG, "HTTP server started on port 80 (7 endpoints registered)");
    }
...
void app_main(void) {
    ESP_LOGI(TAG, "=========================================");
    ESP_LOGI(TAG, "   ESP32 Pixhawk Web Arming Controller");
    ESP_LOGI(TAG, "=========================================");
    ...
    ESP_LOGI(TAG, "=========================================");
    ESP_LOGI(TAG, "Web buttons:");
    ESP_LOGI(TAG, "  SETUP    - Disable pre-arm checks (ARMING_CHECK, failsafes)");
    ESP_LOGI(TAG, "  ARM      - RC override throttle low + arm");
    ESP_LOGI(TAG, "  FORCE ARM- RC override + force arm (magic 21196)");
    ESP_LOGI(TAG, "  DISARM   - Disarm motors");
    ESP_LOGI(TAG, "  Mode     - SET_MODE message (MSG ID 11)");
}
